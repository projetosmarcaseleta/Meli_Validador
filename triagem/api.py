"""
api.py – Chamadas de alta performance à API do Mercado Livre
"""
...
def _session() -> requests.Session:
    if not hasattr(_local, "session"):
        s = requests.Session()
        adapter = HTTPAdapter(pool_connections=10, pool_maxsize=20, max_retries=_RETRY)
...
def get_products_batch(mlbs: list[str], token: str) -> dict:
    ...
    ids_param = ",".join(m.strip().upper() for m in mlbs if m.strip())
